/**
 * Listing types and pure client-side matching logic. Actual data (mock or
 * a live provider) is loaded through lib/listingProviders.ts on the server
 * and delivered to the client via /api/listings.
 */
...
export type Listing = {
  ...
  color: string;
  type: "House" | "Condo";
  /** Optional cover photo URL for live provider data. */
  photo?: string;
  /** Optional link to the source listing (agent, MLS, brokerage). */
  externalUrl?: string;
};
...
export function matchListings(listings: Listing[], criteria: SearchCriteria): Listing[] {
...
/** Static demonstration data used when no real listing provider is configured. */
